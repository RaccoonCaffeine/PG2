...
public Modificar() throws SQLException {
    ...
    int nroIngreso = Integer.parseInt(txtNroIngreso.getText());

    ArrayList<AgregarAclara> listaObj = modDao.rellenadoCampos(nroIngreso);

    // Verificar que la lista no esté vacía antes de acceder a sus elementos
    if (!listaObj.isEmpty()) {
        AgregarAclara obj = listaObj.get(0);  // Suponiendo que solo quieres el primer objeto

        // Configurar los campos con los valores del objeto
        txtNombre.setText(obj.getNombre());
        txtRut.setText(obj.getRut());
        txtInstBancaria.setText(obj.getBanco());
        cbProducto.setSelectedItem(obj.getProducto());

        // Puedes continuar configurando otros campos según sea necesario
        // ...
    }
}
